...
import { generateBundleReadme, generateIntellijEnvironment, generateIntellijHttp, generatePostmanCollection, generatePostmanEnvironment } from "./generators.js";
import { discoverSpecUrl, fetchSpecDocument, parseOpenApiDocument } from "./openapi.js";
async function loadModels(configPath) {
...
export async function generateWorkspace(configPath, outputPath) {
    ...
    await writeFile(path.join(postmanDir, `${config.workspace}.environment.json`), generatePostmanEnvironment(config, models), "utf-8");
    for (const pair of pairs) {
        await writeFile(path.join(intellijDir, `${pair.model.sourceName}.http`), generateIntellijHttp(pair.model, pair.source), "utf-8");
        await writeFile(path.join(postmanDir, `${pair.model.sourceName}.collection.json`), generatePostmanCollection(pair.model, pair.source), "utf-8");
    }
}
