...
export async function discoverSpecUrl(sourceUrl, fetchFn = fetch) {
...
export async function fetchSpecDocument(specUrl, fetchFn = fetch) {
...
function extractAuthSchemes(rawDoc) {
...
function extractOperationAuth(operation, documentSecurity) {
...
function extractRequestBodyExample(operation) {
    ...
    const directExample = jsonContent.example;
...
export function parseOpenApiDocument(document, sourceName) {
    ...
    const title = info && typeof info === "object" && typeof info.title === "string"
        ? String(info.title)
        : sourceName;
    ...
    const endpoints = Object.entries(paths).flatMap(([pathKey, pathItem]) => {
        if (!pathItem || typeof pathItem !== "object") {
            return [];
        }
        const pathObject = pathItem;
